...
import { FaCalendarAlt, FaUsers, FaMapMarkerAlt, FaPhone, FaClock, FaArrowLeft, FaCheckCircle, FaClipboardList, FaClipboardCheck, FaExclamationCircle } from 'react-icons/fa';
...
export default function TaskDetailsPage() {
  ...
  const handleApply = async () => {
    const token = localStorage.getItem("token");
    try {
      const response = await postRequest(`/volunteer/${taskId}/apply`, {}, {
        headers: {
    ...
    >
      ...
      >
        <motion.button 
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          className="bg-indigo-600 text-white px-8 py-4 rounded-xl shadow-lg hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 w-full" 
          onClick={handleApply}
        >
          <FaCheckCircle className="text-xl" />
          <span className="font-medium">Apply for this Task</span>
        </motion.button>
      </motion.div>
      )}
    </motion.div>
