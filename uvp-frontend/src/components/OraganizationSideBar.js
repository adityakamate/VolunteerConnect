...
export default function OraganizationSideBar() {
    ...
    return (
        <div>
            ...
            <aside
                id="logo-sidebar"
                className={`fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform bg-white border-r border-gray-200 shadow-xl ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
                    } sm:translate-x-0`}
                aria-label="Sidebar"
            >
                        ...
                        {SideBarContent.map((content, index) => (
                            <li key={index}>
                                <Link
                                    href={content.path}
                                    onClick={() => setSidebarOpen(false)}
                                    className={`flex items-center p-4 rounded-xl transition-all duration-200 group ${pathname === content.path
                                            ? 'bg-indigo-50 text-indigo-700 border-l-4 border-indigo-500 shadow-sm'
                                            : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                                        }`}
                                >
                                    <span className={`mr-3 text-lg transition-colors duration-200 ${pathname === content.path ? 'text-indigo-600' : 'text-gray-400 group-hover:text-gray-600'
                                        }`}>
                                        {content.icon}
                                    </span>
